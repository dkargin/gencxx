...
# The generated .hxx files should be added ALL_GEN_OUTPUT_FILES_cpp
macro(_generate_msg_cxx ARG_PKG ARG_MSG ARG_IFLAGS ARG_MSG_DEPS ARG_GEN_OUTPUT_DIR)
  ...
  set(MSG_GENERATED_NAME ${MSG_SHORT_NAME}.hxx)
  set(GEN_OUTPUT_FILE ${ARG_GEN_OUTPUT_DIR}/${MSG_GENERATED_NAME})

  # check if a user-provided header file exists
  if(EXISTS "${PROJECT_SOURCE_DIR}/include/${ARG_PKG}/${MSG_SHORT_NAME}.hxx")
    message(STATUS "${ARG_PKG}: Found user-provided header '${PROJECT_SOURCE_DIR}/include/${ARG_PKG}/${MSG_SHORT_NAME}.hxx' for message '${ARG_PKG}/${MSG_SHORT_NAME}'. Skipping generation...")
...
set(gencxx_INSTALL_DIR include)

macro(gencxx_append_include_dirs)
  if(NOT gencxx_APPENDED_INCLUDE_DIRS)
    # make sure we can find generated messages and that they overlay all other includes
    include_directories(BEFORE ${CATKIN_DEVEL_PREFIX}/${gencxx_INSTALL_DIR})
    # pass the include directory to catkin_package()
    list(APPEND ${PROJECT_NAME}_INCLUDE_DIRS ${CATKIN_DEVEL_PREFIX}/${gencxx_INSTALL_DIR})
    set(gencxx_APPENDED_INCLUDE_DIRS TRUE)
  endif()
endmacro()
